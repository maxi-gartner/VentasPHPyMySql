...
INCLUDE ('../app/controllers/categories/read_categories_controller.php');
...
<div class="content-wrapper">
    ...
    <div class="card card-primary col-sm-12">
        ...
        <h3 h3 class="card-title">Lista de categorias</h3>
        </div>
        <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                    ...
                    foreach ($datos_categorias as $dato_categoria) {
                        $id_categoria = $dato_categoria['id'];
                        $nro = $contador += 1;
                        $categoria = $dato_categoria['categoria'];
                        $descripcion = $dato_categoria['descripcion'];
                        ?>
                        <tr>
                        <td><center><?= $nro ?></center></td></td>
                        <td><?= ucfirst($categoria) ?></td>
                        <td><?= ucfirst($descripcion) ?></td>
                        <td>
                            <center>
                            <div class="btn-group">
                            ...
                            <form action="../app/controllers/categories/delete_categorie_controller.php" method="post" style="display: inline;"disabled>
                                <input type="hidden" name="id_usuario" value="<?= $id_categoria ?>">
                                <input type="hidden" name="nombre_usuario" value="<?= $nombre ?>">
                                <button type="button" class="btn btn-danger deleteButton" value="<?= $nombre ?>" id="<?php echo $id_categoria ?>"disabled><i class="fas fa-trash"></i> Borrar</button>
                            </form>
                            </div>
                            </center>
                        </td>
